...
11. **Build Telegram bot** [x] — `bot.py` (774 lines) with all 7 handlers + `SessionState` dataclass + 8 pure helpers (`split_score_args`, `detect_shopee_csv_kind`, `format_score_message`, `format_meta_summary`, `format_uploads_required`, `truncate_for_telegram`, `sanitize_filename`, `build_save_path`). Document upload flow saves files to `data/uploads/{chat_id}_{YYYYMMDD-HHMMSS}_{filename}` with full path-traversal defense. `/insights` command renders `insight_engine.render_summary()`. `tests/test_bot.py` covers all pure helpers (8+4+3+4+3+4+5+3 = 46 test methods, 51 total test cases counting subTest variants). **265/265 green in 3.06s**, 0 warnings.

12. **User provides real credentials** [x] — User pasted `TELEGRAM_BOT_TOKEN` (46 chars) + `MINIMAX_API_KEY` (36 chars) into `hermes-v1/.env`. Sanity check passed: `BASE_URL=https://minimax.afiqstoreapi.cloud/v1`, `MODEL=fiq/minimax-m3`, `bot.py` imports cleanly, `SessionState` exposes the 3 expected fields.

13. **Smoke test live bot** [x] — Fixed nested-loop crash: `main()` is now `def main()` (sync), `app.run_polling(...)` is called without `await`/`asyncio.run()`, and `close_client()` moved into `async def _on_shutdown(app)` registered via `Application.builder().post_shutdown(_on_shutdown)`. Startup confirmed in 4s window: bot reached polling stage, accepted SIGTERM, `_on_shutdown` fired and closed httpx client cleanly. pytest still **265/265 green in 2.98s**. Ready for real Telegram interaction: `/start` → `/help` → `/score ...` → upload 3 CSVs → `/insights`.
